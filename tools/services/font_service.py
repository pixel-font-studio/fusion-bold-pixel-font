...
class FontBuildContext:
    ...
    def load(font_size: FontSize) -> FontBuildContext:
        ...
        for glyph_scope in options.GLYPH_SCOPES:
            context = glyph_file_util.load_context(path_define.FALLBACK_BOLD_GLYPHS_DIR.joinpath(str(font_size), glyph_scope))
            context.update(glyph_file_util.load_context(path_define.ARK_PIXEL_BOLD_GLYPHS_DIR.joinpath(str(font_size), glyph_scope)))
            context.update(glyph_file_util.load_context(path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), glyph_scope)))
            context.pop(-1, None)
