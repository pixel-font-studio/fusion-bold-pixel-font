...
class FontBuildContext:
    ...
    def load(font_size: FontSize) -> FontBuildContext:
        notdef_glyph_file = GlyphFile.load(path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'notdef.png'))

        contexts = {}
        for glyph_scope in options.GLYPH_SCOPES:
            context = glyph_file_util.load_context(path_define.FALLBACK_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope))
            context.update(glyph_file_util.load_context(path_define.ARK_PIXEL_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope)))
            context.update(glyph_file_util.load_context(path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope)))
