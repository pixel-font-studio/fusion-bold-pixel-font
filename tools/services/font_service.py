...
class FontBuildContext:
    ...
    def load(font_size: FontSize) -> FontBuildContext:
        notdef_glyph_file = NamedGlyphFile.load_notdef(path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'notdef.png'))

        cmap_scope_contexts = {
            glyph_scope: CmapContext().merge_by_code_point(
                CmapContext.load(
                    path_define.FALLBACK_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ).with_default_flavor(options.LANGUAGE_FLAVORS),
                CmapContext.load(
                    path_define.ARK_PIXEL_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                CmapContext.load(
                    path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'cmap', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                conflict='replace',
            )
        ...
        named_scope_contexts = {
            glyph_scope: NamedContext().merge_by_name_key(
                NamedContext.load(
                    path_define.ARK_PIXEL_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'named', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                NamedContext.load(
                    path_define.PATCH_BOLD_GLYPHS_DIR.joinpath(str(font_size), 'named', glyph_scope),
                    allowed_flavors=options.LANGUAGE_FLAVORS,
                ),
                conflict='replace',
            )
    ...
    def create_builder(self, width_mode: WidthMode, language_flavor: LanguageFlavor) -> FontBuilder:
        ...
        builder.meta_info.family_name = f'Fusion Bold Pixel {self.font_size}px {width_mode[:4].capitalize()} {configs.LANGUAGE_FLAVOR_TO_FONT_NAME[language_flavor]}'
        ...
        builder.meta_info.description = 'Open-source Pan-Latin and Pan-CJK pixel font in a bold sans-serif style'
        builder.meta_info.copyright_info = 'Copyright (c) 2026, TakWolf (https://takwolf.com)'
        builder.meta_info.license_info = 'This Font Software is licensed under the SIL Open Font License, Version 1.1'
        builder.meta_info.vendor_url = 'https://pixel-font-studio.github.io/fusion-bold-pixel-font'
        builder.meta_info.designer_url = 'https://takwolf.com'
        builder.meta_info.license_url = 'https://github.com/pixel-font-studio/fusion-bold-pixel-font/blob/master/LICENSE-OFL'
        ...
        if len(font_formats) > 0:
            for language_flavor in options.LANGUAGE_FLAVORS:
                ...
                for font_format in font_formats:
                    file_path = path_define.OUTPUTS_DIR.joinpath(f'fusion-bold-pixel-{self.font_size}px-{width_mode}-{language_flavor}.{font_format}')
