...
def bolding_glyphs(font_size: FontSize) -> None:
    root_dirs = [
        (
    ...
    for source_root_dir, target_root_dir in root_dirs:
        ...
        logger.info('Bolding glyphs: {!r} -> {!r}', str(source_root_dir), str(target_root_dir))
        for source_file_path in source_root_dir.rglob('*.png'):
